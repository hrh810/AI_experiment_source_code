...
# ============变量初始化==============
# 年龄段和性别
# 使用列表推导式生成包含数字 0 到 100 的列表
ageList = [str(num) for num in range(101)]
# ageList = [str(num) for num in range(20, 31)]
...
#         if confidence > 0.2:  # 筛选一下，将置信度大于0.7侧保留，其余不要了
...
while True:
    # 读一帧
    frame = cv2.imread('images/myself.jpg')
    ...
    frame, faceBoxes = getBoxes(frame)
